/*** BEGIN META {
  "name" : "UC_RefHelper_findBuildsWhereParam",
  "comment" : "Returns a formatted HTML list of build href_links related by a common build parameter whose value is defined by the user.",
...
vShow       : Flag indicating whether to display links
*/
...
jenkins.model.Jenkins.instance.getItem(job).getBuilds().each{
  ...
  if (commonParam==vValue && it.result.toString()=='SUCCESS'){
    ...
    uc_value="(${it.id})_${it.getDisplayName()}"
